...
class TestSubscriptionNegative:
    ...
    def test_creating_subscription_with_unverified_email(
        self, logged_in_user, api_manager, get_list_subscriptions
    ):
        ...
        with allure.step("Запрос на формирование оплаты"):
            response = api_manager.subscriptions_api.subscriptions_payment_pending(
                id_subscriptions, expected_status=403
            ).json()
        ...
        with allure.step("Assert"):
            assert validate_response.description == "Route is available for verified users!", (
                "Описание не совпадает"
            )

    @allure.title("Удаление подпискии неподтвержденным пользователем")
    def test_delete_subscription_unverified_user(
        self, logged_in_user, api_manager, get_list_subscriptions
    ):
        ...
        with allure.step("Запрос на удаление подписки"):
            response = api_manager.subscriptions_api.delete_subscriptions(
                request_body, expected_status=403
            ).json()
    ...
    def test_delete_non_existent_subscription(self, static_user, api_manager):
        ...
        with allure.step("Запрос на удаление подписки"):
            response = api_manager.subscriptions_api.delete_subscriptions(
                request_body, expected_status=404
            ).json()
        ...
        with allure.step("Assert"):
            assert validate_response.description == "Subscription not found", (
                "Описание не совпадает"
            )
