...
from constants.constants import PAYMENT_SUBSCRIPTIONS_URL, NAME_SUBSCRIPTIONS
...
from models.Subscriptions.model_user_subsriptions import UserSubscriptionResponse, ModelErrorResponse
...
@allure.feature("Получение подписок у пользователя, "
...
class TestSubscriptionPositive:
    ...
    def test_subscription_pyments(self, static_user, api_manager, get_list_subscriptions):
        ...
        with allure.step("Отправка запроса на оплату подписки"):
            response = api_manager.subscriptions_api.subscriptions_payment_pending(id_subscriptions, static_user.email).text
    ...
    def test_payment_details(self, payment_link_subscriptions, static_user):
        ...
        with allure.step("Assert"):
            ...
            check.equal(payment_details.templateParams.merchantPhone, "9183374202", "Номер телефона не совпадает")
            check.equal(payment_details.templateParams.companyName, "ИП КУЯНЕЦ РУСЛАН РОСТИСЛАВОВИЧ", "Получатель не совпадает")
            check.equal(payment_details.templateParams.companyAddress, "ул им. генерала И.Л. Шифрина, д 1", "Получатель не совпадает")
            ...
            check.equal(payment_details.order.description, "Оплата подписки 'Премиум на 3 месяца'", "Описание не совпадает")
            check.equal(payment_details.customer.key, static_user.id, "ID заказчика не совпадают")
            check.equal(payment_details.customer.email, static_user.email, "Email заказчика не совпадают")
            assert datetime.datetime.fromisoformat(payment_details.status.timestamp), "Неверный формат даты"

    @allure.title("Проверка статуса подписки после создания оплаты")
    def test_status_subscription_after_payment(self, payment_link_subscriptions, api_manager, static_user):
    ...
    def test_delete_subscription(self, static_user, api_manager, payment_link_subscriptions):
        ...
        with allure.step("Получаем обновленный список подписок после удаления"):
            updated_subscriptions = api_manager.subscriptions_api.get_subscriptions_users(static_user.id).json()

        with allure.step("Валидируем Обновленные данные через пудантик"):
            validate_updated = DataUtils.type_adapter(List[UserSubscriptionResponse] ,updated_subscriptions)
    ...
    def test_free_subscription(self, api_manager, static_user, get_list_subscriptions):
        ...
        with allure.step("Делаем запрос на создание оплаты с беcплатной подпиской"):
            response = api_manager.subscriptions_api.subscriptions_payment_pending(subscription_id, expected_status=400).json()

        with allure.step("Валидация ответа"):
            validate_response = ModelErrorResponse.model_validate(response)

        with allure.step("Assert"):
            assert "Неверная сумма. Сумма должна быть больше или равна 100 копеек." in validate_response.description, "Описание отсутствует"

    @allure.title("Повторный запрос на оплату подписки")
    def test_retry_subscription_payment(self, static_user, api_manager, get_list_subscriptions, payment_link_subscriptions):
        with allure.step("Получаем ID подписки"):
            subscription_id = DataUtils.find_item(
                items=get_list_subscriptions,
                condition=lambda sub: sub.name == NAME_SUBSCRIPTIONS,
        ...
        with allure.step("Делаем повторный запрос на создание ссылки на оплату"):
            response = api_manager.subscriptions_api.subscriptions_payment_pending(subscription_id, static_user.email).text
