...
from tests.mail.verification_flow import authenticate_for_teardown, assert_profile_not_verified
...
def cleanup_user(api_manager: ApiManager, test_user: dict) -> None:
    ...
    for pwd in candidates:
        ...
        delete_resp = api_manager.user_api.delete_user(
            user_id, expected_status=[200, 204, 404]
        )
...
def create_unverified_mail_registered_user(api_manager: ApiManager, test_user: dict) -> dict:
    ...
    _refresh_mail_identity = _make_mail_identity_refresher(mail_identity,
                                                           test_user)
...
def create_verified_mail_registered_user(api_manager: ApiManager, test_user: dict) -> dict:
    ...
    """
    mail_identity: dict = {"started_at": None}
    refresh_mail_identity = _make_mail_identity_refresher(mail_identity,
                                                          test_user)
